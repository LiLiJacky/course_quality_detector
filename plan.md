...
- **后摄优先，前摄补漏**：按摄像头分组（如 509B 后摄、509A 前摄），后摄识别占主权重，前摄仅做补漏。  
- **分数权重**：总分 100，将后摄命中计权重（如 70 分），前摄补充计权重（如 30 分）；仅前摄命中时采用单独补漏权重（如 60 分）。  
- **判定规则**：  
  1) 统计每个 ID 的出现次数与平均相似度（仅当相似度 ≥ 阈值）分别在后摄、前摄。  
  2) 若后摄满足 `count >= min_count_back` 且 `mean_sim >= thr_back`，给后摄得分。  
  3) 前摄补充：`count >= min_count_front` 且 `mean_sim >= thr_front` 时给前摄补分，与后摄分相加。  
  4) 若无后摄命中，前摄补漏需更高要求（如 `count >= min_count_front_only`、`mean_sim >= thr_front_only`、且全局计数达到 min_recognized_counts_front_only），得到补漏分。  
  5) 最终出勤：后摄+前摄分 ≥ 出勤阈值，或前摄补漏分 ≥ 出勤阈值，才计为到场。  
- **可配置参数**：后/前摄关键词、各自阈值与 min_count、权重（后摄/前摄/前摄补漏）、出勤分阈值、全局最小计数过滤（min_recognized_counts）。
